...
public class EmailResource {
     ...
     */
    @GetMapping("")
    public List<EmailDTO> getAllEmails(@RequestParam(name = "filter", required = false) String filter) {
        if ("pmentreprise-is-null".equals(filter)) {
            LOG.debug("REST request to get all Emails where pmEntreprise is null");
            return emailService.findAllWherePmEntrepriseIsNull();
        }

        if ("pmetablissement-is-null".equals(filter)) {
            LOG.debug("REST request to get all Emails where pmEtablissement is null");
            return emailService.findAllWherePmEtablissementIsNull();
        }
